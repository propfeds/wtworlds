...
execute unless score @s wtworlds.initiate.fly matches 1.. if entity @s[x_rotation=-20..90] run return 0

execute rotated ~ 0 if block ^ ^1 ^1.5 #taglib:walk_through run teleport @s ^ ^ ^0.75
execute at @s rotated ~ 0 unless block ~ ~1 ~ #taglib:walk_through run teleport @s ^ ^ ^-0.5

scoreboard players add @s wtworlds.initiate.fly 1
execute unless score @s wtworlds.initiate.fly matches 3.. run return 0

execute if entity @s[x_rotation=25..90] run return run function wtworlds:entity/initiate/reset_flight
